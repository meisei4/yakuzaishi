use amethyst::{
    ...
    renderer::{
        SpriteRender,
        Transparent, sprite::SpriteSheetHandle,
    },
};

use tiled::{FiniteTileLayer, LayerTile};

use crate::{TILE_SIZE, resources::game_map_resource::GameMapResource};
            ...
            match layer.layer_type() {
                tiled::LayerType::Tiles(tile_layer) => match tile_layer {
                    tiled::TileLayer::Finite(finite_layer) => {
                        process_finite_layer(&finite_layer, &entities, &game_map.sprite_sheet_handle, &lazy_update);
                    }
    ...
    for y in 0..finite_layer.height() {
        for x in 0..finite_layer.width() {
            if let Some(tile) = finite_layer.get_tile(x as i32, y as i32) {
                ...
                let sprite_render: SpriteRender = create_sprite_render_for_tile(tile, sprite_sheet_handle);
...
fn create_sprite_render_for_tile(tile: LayerTile, sprite_sheet_handle: &SpriteSheetHandle) -> SpriteRender {
    SpriteRender {
