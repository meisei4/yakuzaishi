...
impl GameMapResource {
    pub fn new(world: &mut World, map_file_path: &str, tsx_file_path: &str, texture_file_path: &str) -> Self {
        ...
        info!("Loading TMX map file: {}", map_file_path);
        let tiled_map: tiled::Map = tiled_loader
            .load_tmx_map(Path::new(map_file_path))
            .expect("Failed to load tilemap");
        
        info!("TMX map file loaded successfully.");
        info!("Loading texture for sprite sheet: {}", texture_file_path);
        ...
        info!("Loading TSX tileset file: {}", tsx_file_path);
        let tileset: tiled::Tileset = tiled_loader
            .load_tsx_tileset(Path::new(tsx_file_path))
            .expect("Failed to load tileset");
        info!("TSX tileset file loaded successfully.");

        info!("Converting tileset to sprite sheet.");
        let sprite_sheet_data: SpriteSheet = util::convert_tileset_to_sprite_sheet(&tileset, &texture_handle);   
        info!("Tileset converted to sprite sheet successfully.");

        info!("Creating sprite sheet handle.");
        let sprite_sheet_handle: Handle<SpriteSheet> = asset_loader.load_from_data(
            sprite_sheet_data, 
            (),
            &sprite_sheet_storage
        );
        info!("Sprite sheet handle created successfully.");

        GameMapResource { tiled_map, sprite_sheet_handle }
    }
}
